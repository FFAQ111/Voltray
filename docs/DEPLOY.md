...
> **Upgraded to v3 (2026-06-22, compatible upgrade).** `published-at = 0x6057edd8a8438489ee52390594b0cf408a48d368c9215a9ebbea7b61be9e3efa`; `original-id` unchanged. Body-only change: `settle` / `reclaim_remaining` now accept `ctx.sender() == event.utility` **or** a hardcoded `ORACLE` address, so the hosted daemon (one key) can close out events created by *any* wallet — a judge's zkLogin event included. Charger-signature and reclaim-to-`event.utility` invariants are untouched, so this grants the oracle no power to fabricate readings or redirect funds. **Adoption:** `fly secrets set PACKAGE_AT=0x6057edd8…` (and `oracle/src/config.ts` default bumped to match). The daemon's `closeAllEnded` no longer filters on `utility === oracleAddr`. The **frontend needs no change** — its reclaim call runs as the creator (utility), allowed under both the old and new body.
